...
//         .then(books => {
//             console.log('books.data.items', books.data.items);
...
function addGoogleBook(googleBook) {
    const book = {
        ...
        listPrice: {
            amount: utilService.getRandomInt(20, 200),
            currencyCode: 'USD',
            isOnSale: true,
        },
        reviews: [],
    }
    if (googleBook.saleInfo.saleability === "NOT_FOR_SALE") book.listPrice.isOnSale = false;
...
function searchBookFromAPI(searchBook) {
    const searchRes = utilService.loadFromStorage(searchBook);
    if (searchRes && searchRes.length) return Promise.resolve(searchRes);
    return axios.get(`https://www.googleapis.com/books/v1/volumes?q=${searchBook}`)
        .then(books => {
            const currBooks = books.data.items;
            utilService.saveToStorage(searchBook, currBooks);
            return currBooks;
        })
}
